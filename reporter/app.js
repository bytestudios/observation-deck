...
import {user} from '/reporter/store/user.js'             //user state
import {data} from '/reporter/store/data.js'             //user state
...
const app = Vue.createApp({
	...
	<q-layout view="hHh LpR fFf">
		<q-toolbar v-if="data.currentPath!='/login/'"  :class="(data.currentPage=='All Session Data') ? 'bg-grey-1 add-logo' : 'bg-grey-1'">
			<!-- <q-btn v-if="data.currentPath=='/'" flat round dense icon="menu" /> -->
				...
				<q-menu auto-close>
					<q-list style="min-width: 200px">
						...
						<q-item clickable @click="user.loadLatestData()">
							...
							<q-item-section style="width: 90%"><q-toggle id="dark_toggle" color="secondary" v-model="dark_mode" true-value="dark" false-value="light" /></q-item-section>
						</q-item>
						<q-item clickable @click="user.loadLatestData()">
							<q-item-section avatar style="min-width: 20px">
		...
		<!-- Main content -->
		<q-page-container v-if="data.currentPath=='/'" class="q-pa-lg" :class="$q.dark.isActive ? 'bg-grey-8' : 'bg-grey-2'" >
